...
exports.getPremium = async (req, res, next) => {
  try {
    const Razor = new Razorpay({
      //i have created new object called Razor and here my backend interns connect to the razorpay
...
function generateToken(id, ispremiumUser) {
  return jwt.sign(
    { userId: id, isPremium: ispremiumUser },
    "fiuhf2bd484fdfhfff656ffhfEwddfkmnv"
  );
}
...
exports.updatePremium = async (req, res, next) => {
  try {
    ...
    Promise.all([updateTable(result), updateUserTable()]).then(() => {
      ...
      res.json({
        success: true,
        message: "Premium Purchased Successfullly",
        token: generateToken(req.user.id, true),
      });
